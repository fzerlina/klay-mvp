...
function makeCustomer(idx) {
  ...
  // Ship-to: empty means "same as billing". Every 5th company customer keeps a
  // separate warehouse. Derived from idx rather than chance() so this does NOT
  // consume the RNG stream — the seed 20250424 dataset stays reproducible.
  const shippingAddress = !isIndividual && idx % 5 === 0
    ? `Warehouse — ${STREETS[idx % STREETS.length]} No. ${100 + (idx % 90)}, ${CITIES[idx % CITIES.length]} ${20000 + idx * 7}`
    : "";
  return {
    id, code, type,
    name, legalName,
...
function makeInvoice(idx) {
  ...
  const settled = !isDraft && !overdue && chance(0.7);
  const approval = isDraft ? "draft" : "sent";
  const payStatus = isDraft ? "unpaid" : (settled ? "paid" : (overdue ? "overdue" : "unpaid"));
...
for (const c of customers) {
  ...
  c.ar = myInvs.filter((i) => i.payStatus !== "paid" && i.approval !== "draft")
...
for (const i of invoices) {
  if (i.total !== i.dpp) errors.push(`${i.id}: total ${i.total} != dpp ${i.dpp}`);
...
const totalInvAR    = invoices.filter((i) => i.payStatus !== "paid" && i.approval !== "draft").reduce((s, i) => s + i.total, 0);
